...
class PartnerDetailsPopup extends ConsumerStatefulWidget {
  const PartnerDetailsPopup(
      {super.key, this.partner, this.initialStep, this.callback});
...
class _PartnerDetailsPopupState extends ConsumerState<PartnerDetailsPopup> {
  ...
  void initState() {
    ...
    currentStep = widget.initialStep ?? currentStep;
  }
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  widget.partner != null && widget.initialStep != null
                      ? Text(
                          '${'edit'.tr(context)} '
                          '${titleList[widget.initialStep!].toLowerCase()}',
    ...
    return SingleChildScrollView(
      child: Form(
        child: Column(
          ...
          children: [
            CustomTextField1(
              initialValue: _partner.bankAccount?.bank,
              keyboardType: TextInputType.name,
              labelText: 'bank'.tr(context),
              hintText: 'bank_hint'.tr(context),
              onValueChanged: (String value) {
                _partner.bankAccount?.bank = value;
              },
            ),
            Gap(4),
            CustomTextField1(
              initialValue: _partner.bankAccount?.iban,
              keyboardType: TextInputType.name,
              labelText: 'iban'.tr(context),
              hintText: 'iban_hint'.tr(context),
              onValueChanged: (String value) {
                _partner.bankAccount?.iban = value;
              },
            ),
          ],
        ),
